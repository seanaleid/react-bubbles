...
You can keep your data secure by using PrivateRoutes. These routes only allow users with the appropriate token to enter those protected areas/pages/components. Additionally, I've heard you can assign roles to users with tokens so that there is another layer of security. Not sure how that works yet, but essentially you can have a user role that can view information (according to the type of token they have) or an admin role that can virtually do everything (delte, update, add --> all the CRUD, and again based on what kind of token they have). 
...
Create --> .POST   --> It adds info that a user created to the server.
Read   --> .GET    --> This fetches info from the server to render on screen.
Update --> .PUT    --> This replaces the current data on the server with a new request payload, usually from inputs.
Delete --> .DELETE --> This deletes the information from the server.
